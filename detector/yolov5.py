...
class YoloV5(Model):
    ...
    def inference(self: YoloV5, sess: Session) -> np.ndarray:
        ...
        # confidence of bbox itself and confidences for each class
        oconf = pred[:, 4:5]
        cconf = pred[:, 5:]
        # confidences for each class should be multiplied by object conf
        cconf *= oconf
        # class ids
        cls = cconf.argmax(axis=1)[:, np.newaxis].astype(np.float)
        # confidences of each bouding box for its most plausible class
